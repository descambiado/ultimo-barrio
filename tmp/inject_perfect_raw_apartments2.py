...
# Clean scene of ANY previous apartments
scene_path = 'Assets/scenes/main.scene'
d = json.load(open(scene_path, 'r', encoding='utf-8'))
new_root = []
for go in d['GameObjects']:
    if 'Apartment' in go.get('Name', '') or 'apartment' in go.get('Name', ''):
        continue
    if go.get('Name') == 'World':
        if 'Children' in go:
            go['Children'] = [c for c in go['Children'] if 'Apartment' not in c.get('Name', '') and 'apartment' not in c.get('Name', '')]
    new_root.append(go)
...
print('Injected PERFECT raw clones for A01 and A02, removed all others.')
