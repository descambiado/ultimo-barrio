...
| 8 | `8eb4253` | feat(content): fortificación (9 prefabs) | `Assets/prefabs/content/fortification/*.prefab` | Hosts de fortificación (commit 3) | Modelos REALES del engine verificados (asset_search + asset_info IsCompiled): barricada madera→`old_bench`, barricada ref→`security_shutter_box_middle`, puertas→`security_shutter_curtain_128/bottom` (proxy visual: no hay puertas vmdl en engine), stash→`gritbin01_combined`, workbench→`oldoven` (proxy), generator→`aircon_unit_medium_wall` (proxy), alarm→`intruder_alarm_2`, repair station→`microwave_trans` (proxy). Registry: sección building añadida | IDs de objeto vs housing/ownership del core | **SÍ** (JSON puro) | 3 |
