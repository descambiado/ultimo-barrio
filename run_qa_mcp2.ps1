function Call-Mcp ($method, $argsJson) {
    $json = '{ "jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": { "name": "' + $method + '", "arguments": ' + $argsJson + ' } }'
    
    $response = Invoke-RestMethod -Uri "http://127.0.0.1:7269/mcp" -Method Post -Body $json -ContentType "application/json" -ErrorAction SilentlyContinue
...
Write-Host (Call-Mcp "read_console" "{}")
